...
export function ColumnsSection({
  ...
  const [listsToReorderCloned, setListsToReorderCloned] = React.useState<Record<
    string,
    ListType
  > | null>(null);
  ...
  return (
    ...
    >
      ...
      <DragOverlay>
        {activeCard && <Card isOverlay card={activeCard} boardId={boardId} />}
      </DragOverlay>
    </DndContext>
  );
  ...
  function handleDragStart(event: DragStartEvent) {
    const data = event.active.data as React.MutableRefObject<DraggableCardData>;
      ...
      if (over.id in listsToReorder) {
        ...
        if (listsToReorder[over.id].cards.length === 0) {
          ...
          ({ srcList, destList } = moveCardBetweenLists({
            srcList: listsToReorder[data.current.listId] as ListWithId,
        ...
        if (active.id !== over.id && overData.current) {
          const { srcList, destList } = moveCardBetweenLists({
            srcList: listsToReorder[data.current.listId] as ListWithId,
            destList: listsToReorder[overData.current.listId] as ListWithId,
            srcIndex: data.current.index,
