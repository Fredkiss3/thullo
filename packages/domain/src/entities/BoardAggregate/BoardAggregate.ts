...
export class BoardAggregate {
  ...
  addLabelToCard(
    ...
    if (labelId === undefined) {
      if (name !== undefined && color !== undefined) {
        ...
        throw new Error('You must provide a name and a color for the label');
      }
    } else {
      const label = this._labelsById[labelId];

      if (label === undefined) {
        throw new LabelNotFoundError("this label doesn't exist in the board");
      }

      const alreadyExistingLabel = card.labels.find(({ id }) => id === labelId);
  ...
  addMemberToBoard(member: Member): void {
    if (
      this._participants.find(({ member: { id } }) => id === member.id) ===
      undefined
    ) {
      this._participants = [...this._participants, { isAdmin: false, member }];
    }
  }
  ...
  addList(name: string): ListId {
    ...
    if (Object.keys(this._listsById).length > 0) {
      newPosition =
        Math.max(
          ...Object.values(this._listsById).map(({ position }) => position)
        ) + 1;
    }
  ...
  addCardToList(title: string, listId: ListId): Card {
    ...
    if (list === undefined) {
      throw new ListNotFoundError("cette liste n'existe pas dans le tableau");
    }
  ...
  removeMemberFromBoard(member: Member, initiatorId: MemberId) {
    ...
    if (memberFound === undefined) {
      throw new MemberNotInBoardError("Ce membre n'est pas dans ce tableau");
  ...
  moveCard(
    ...
    for (const listId in this._cardsByListIds) {
      const card = this._cardsByListIds[listId].find(({ id }) => id === cardId);
    ...
    if (cardToMove === undefined) {
      throw new CardNotFoundError("cette carte n'existe pas dans le tableau");
    }
